...
def show_event_function():
    if venue_url == '':
        ...
        venue_title = get_site_title(venue_url)
        site = download(url=venue_url)
        if site != None:
            event_title, event_date = get_page_data(venue_title)
...
def get_page_data(venue_title):
    site_data = unescape(open('downloaded_document.html', 'r').read())
    if 'Tivoli' in venue_title:
        event_title, event_date = get_tivoli_data(site_data)
    elif 'Brisbane' in venue_title:
        event_title, event_date = get_brisbane_data(site_data)
    else:
        event_title, event_date = get_suncorp_data(site_data)

    return event_title, event_date

def get_tivoli_data(site_data):
    event_title = findall('class="image-title">(.+)</div>', site_data)[0]
    event_date = findall('class="image-subtitle">\n\t\t\t\t\t\t(.+)\t\t\t\t\t<', site_data)[0]
    event_date = str(event_date).split("<", 1)
    
    return event_title, event_date[0]

def get_brisbane_data(site_data):
    event_title = findall('name--content">(.+)</div>', site_data)[0]
    event_title = str(event_title).split("<", 1)
    event_date = findall('--heavy eds-text-bm">(.+)</div>', site_data)
    event_date = str(event_date).split("<", 1)
    event_date = str(event_date[0]).strip("[")
    event_date = str(event_date).strip("'")

    return event_title[0], event_date

def get_suncorp_data(site_data):
    event_title = findall('class="event-title">(.+)</h6>', site_data)[0]
    event_date = findall('class="event-date text-uppercase">(.+)</h7>', site_data)[0]

    return event_title, event_date

def get_site_title(venue_url):
...
values = {"The Tivoli theatre": ['https://thetivoli.com.au/events', 0],
          "Brisbane City": ['https://www.eventbrite.com.au/d/australia--brisbane-city/events/', 1],
          "Suncorp Stadium": ['https://suncorpstadium.com.au/what-s-on.aspx', 2]}
